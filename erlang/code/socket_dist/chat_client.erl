...
-import(io_widget, 
	[get_state/1, insert_str/2, set_prompt/2, set_state/2, 
	 set_title/2, set_handler/2, get_group_names/2, get_all_groups/2,  update_state/3]).

-export([start/0, test/0, connect/5]).


% update_group_list() -> lists:append(Nick, list).
...
test() ->
	...
	% connect("localhost", 9000, "12345", "2nd", "jim"),
    % connect("localhost", 9000, "12345", "2nd", "sue").
...
active(Widget, MM) ->
	 ...
	 {Widget, Nick, Str} ->
	     ...
	     active(Widget, MM);
	 {chan, MM, {groups, List}}  ->
		 get_all_groups(Widget, [List]),
		 active(Widget, MM);
 	 {chan, MM, {names, List}}   ->
		 get_group_names(Widget, [List]),
		 active(Widget, MM);
	 {chan, MM, {msg, From, Pid, Str}} ->
	     insert_str(Widget, [From,"@",pid_to_list(Pid)," ", Str, "\n"]),
	     active(Widget, MM);
	 {chan, MM, {oneToOne, From, Pid, Str}} ->
	     insert_str(Widget, ["\n++Personal Chatting With ",From,"++\n",From,"@",pid_to_list(Pid)," ", Str, "\n"]),
	     active(Widget, MM);
	 ...
	 {close, MM} ->
	     exit(serverDied);
	 {Widget, From, To, Str}  ->
		%  io:format("widget, from, to str"),
		 lib_chan_mm:send(MM, {oneToOne, From, To, Str}),
		 active(Widget, MM);
	 Other ->
